...
export class Migration20230604164614_Categories extends Migration {
	async up(): Promise<void> {
		...
		this.addSql(
			'alter table "purchases" add constraint "purchases_category_id_foreign" foreign key ("category_id") references "categories" ("id") on update cascade;'
		);
		this.addSql(
			'alter table "purchases" add constraint "purchases_supplier_id_foreign" foreign key ("supplier_id") references "suppliers" ("id") on update cascade;'
		);
